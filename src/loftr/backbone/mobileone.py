...
class MobileOneBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if inference_mode:
            ...
            # Re-parameterizable skip connect
            # ion
...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        # Build stages
        self.stage0 = MobileOneBlock(in_channels=1, out_channels=self.in_planes,
        ...
        self.stage2 = self._make_stage(int(128 * width_multipliers[1]), num_blocks_per_stage[1],
                                       num_se_blocks=0)
        self.conv = nn.Conv2d(in_channels=int(128 * width_multipliers[1]), out_channels=256, kernel_size=1, stride=1,groups=64)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                if m.bias is not None:
                    nn.init.zeros_(m.bias)  # 将bias初始化为0
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x2 = self.stage2(x1)
        x2 = self.conv(x2)
        return {'feats_c': x2, 'feats_f': None, 'feats_x2': x1, 'feats_x1': x}
...
def mobileone(inference_mode: bool = False,
    ...
    return MobileOne(inference_mode=inference_mode,
                     **variant_params)
