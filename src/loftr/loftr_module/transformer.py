...
class AG_RoPE_EncoderLayer(nn.Module):
    def __init__(self,
        ...
        self.aggregate = nn.Conv2d(d_model, d_model, kernel_size=agg_size0, padding=0, stride=agg_size0, bias=False,
                                   groups=d_model) if self.agg_size0 != 1 else nn.Identity()
        self.max_pool = torch.nn.MaxPool2d(kernel_size=self.agg_size1,
                                           stride=self.agg_size1) if self.agg_size1 != 1 else nn.Identity()
        if self.rope:
        ...
        self.local_mixer = nn.Conv2d(d_model, d_model, kernel_size=3, padding=1, stride=1,
                                     bias=False, groups=d_model) if self.agg_size0 != 1 else nn.Identity()
        self.mixer = nn.Conv2d(d_model, d_model, 1, 1, 0)
        # feed-forward network
        self.mlp = ConvFFN(d_model * 2, d_model, kernel_size=3, stride=1, out_channels=d_model)
    ...
    def forward(self, x, source, x_mask=None, source_mask=None):
        ...
        query, source = self.norm1(self.aggregate(x).permute(0, 2, 3, 1)), self.norm1(
            self.max_pool(source).permute(0, 2, 3, 1))  # [N, H, W, C]
        ...
        query_local = query.permute(0, 3, 1, 2)
        # Positional encoding        
        if self.rope:
        ...
        global_feat = rearrange(m, 'b (h w) c -> b c h w', h=H0 // self.agg_size0,
                                w=W0 // self.agg_size0)  # [N, C, H0, W0]

        local_feat = self.local_mixer(query_local)
        ...
        m = self.mixer(local_feat * global_feat)
        # Upsample feature
        if self.agg_size0 != 1:
            m = torch.nn.functional.interpolate(m, scale_factor=self.agg_size0, mode='bilinear',
                                                align_corners=False)  # [N, C, H0, W0]

        # feed-forward network
        m = self.mlp(torch.cat([x, m], dim=1))  # [N, C, H0, W0]
        return x + m
