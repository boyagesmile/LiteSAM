...
def spvs_fine(data, config, logger=None):
    ...
    if m == 0:  # special case: there is no coarse gt
        ...
        if config.LOFTR.ALIGN_CORNER is False:  # even windows
            assert W == 8
            grid_pt0_f_unfold = F.unfold(grid_pt0_f, kernel_size=(W, W), stride=W, padding=0) # 在832*832上以8*8滑窗
        grid_pt0_f_unfold = rearrange(grid_pt0_f_unfold, 'n (c ww) l -> n l ww c', ww=W ** 2)  # [1, hc0*wc0, W*W, 2]
        grid_pt0_f_unfold = repeat(grid_pt0_f_unfold[0], 'l ww c -> N l ww c', N=N)  # [N, hc0*wc0, W*W, 2]

        # 2. select only the predicted matches
        grid_pt0_f_unfold = grid_pt0_f_unfold[data['b_ids'], data['i_ids']]  # [m, ww, 2] # img0中待匹配点的位置
        grid_pt0_f_unfold = scalei0[:, None, :] * grid_pt0_f_unfold  # [m, ww, 2] # 回到输入图像尺寸
        ...
        for b in range(N):
            ...
            match = int(mask.sum())
            correct_0to1_f_mask, w_pt0_i_mask = warp_kpts(grid_pt0_f_unfold[mask].reshape(1, -1, 2),
                                                          data['depth0'][[b], ...],
                                                          data['depth1'][[b], ...], data['T_0to1'][[b], ...],
                                                          data['K0'][[b], ...],
                                                          data['K1'][[b], ...])  # [k, WW], [k, WW, 2] 将img0的点投影到img1
        ...
        delta_w_pt0_i = w_pt0_i - pt1_i[b_ids, j_ids][:, None, :]  # [m, WW, 2] 像素级别的gt偏差
        del b_ids, i_ids, j_ids
        delta_w_pt0_f = delta_w_pt0_i / scalei1[:, None, :] + W // 2 - 0.5  # 归一化偏差，局部网格中相对位置
        ...
        i_ids_di, i_ids_dj = i_ids // W, i_ids % W
        m_ids, i_ids, j_ids_di, j_ids_dj = m_ids.to(torch.long), i_ids.to(torch.long), j_ids_di.to(
            torch.long), j_ids_dj.to(torch.long)

        # expec_f_gt will be used as the gt of subpixel-level refinement
        expec_f_gt = delta_w_pt0_f - delta_w_pt0_f_round  # 亚像素级
        ...
        if m_ids.numel() == 0:  # special case: there is no pixel-level gt
            ...
            data.update({"m_ids_f": m_ids,
                         ...
                         "i_ids_f_di": i_ids_di,
                         "i_ids_f_dj": i_ids_dj,
                         })  #如果找不到有效的匹配点，会生成默认的全零值，否则将正确的真值存储在 data 中。
