...
class RoPEPositionEncodingSine(nn.Module):
    ...
    def __init__(self, d_model, max_shape=(256, 256), npe=None, ropefp16=True):
        ...
        sin[:, :, 0::2] = torch.sin(i_position * div_term).half() if ropefp16 else torch.sin(i_position * div_term)  # 偶数索引
        sin[:, :, 1::2] = torch.sin(j_position * div_term).half() if ropefp16 else torch.sin(j_position * div_term)  # 奇数索引
