...
class ScanNetDataset(utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image0, color0 = read_scannet_color(img_name0, resize=self.img_resize, augment_fn=None)
        #    augment_fn=np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
        image1, color1 = read_scannet_color(img_name1, resize=self.img_resize, augment_fn=None)
        #    augment_fn=np.random.choice([self.augment_fn, None], p=[0.5, 0.5]))
